...
#
# XFCE binds Ctrl+Super+L to xflock4, the chord this i3 profile uses for numeric
# workspace navigation. Only that custom override goes; XFCE's default Ctrl+Alt+L
# stays.
#
# xfsettingsd keeps the X11 passive grab after xfconf drops the property, so the
# daemon is replaced to rebuild its shortcut table before i3 reloads to claim the
# chord. On that reload the property is already absent, so this does not recurse.
